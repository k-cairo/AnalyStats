    ...
    def get_iframes(self) -> None:
        ...
        if self.status.success:
            for season in self.ACTIVE_SEASONS:
                ...
                # Get Url
                self.get(url=f"{season.url}over-3-5-goals/", error_context=f"{self.ERROR_CONTEXT}.get_iframes()")
                if not self.status.success:
                    continue

                # Get BTTS Iframes
                over_35_goals_iframes: ResultSet[Tag] = self.soup.select(selector="div.tab-content iframe")
                ...
                for idx, over_35_goals_iframe in enumerate(over_35_goals_iframes):
                    try:
                        ...
                        self.exception(
                            error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_OVER_35_GOALS_IFRAME_URL_FAILED,
                ...
                if not iframe.exists():
                    iframe.save()
                    self.log_info(f"League {season.league.name} Over 3.5 Goals Iframe created in database")
                else:
                    target_iframe: E5Over35GoalsIframe = E5Over35GoalsIframe.objects.get(season=season)
                    ...
                    target_iframe.save()
                    self.log_info(f"League {season.league.name} Over 3.5 Goals Iframe updated in database")
            ...
            for iframe in self.OVER_35_GOALS_IFRAMES:
                ...
                for table_tr in table_trs:
                    ...
                    if not home_over_35_goals_stats.exists():
                        ...
                        self.log_info(f"Team {home_team.name} Over 3.5 Goals Home Stats created in database")
                    else:
                        ...
                        self.log_info(f"Team {home_team.name} Over 3.5 Goals Home Stats updated in database")
                    ...
                    if not away_over_35_goals_stats.exists():
                        ...
                        self.log_info(f"Team {away_team.name} Over 3.5 Goals Away Stats created in database")
                    else:
                        ...
                        self.log_info(f"Team {away_team.name} Over 3.5 Goals Away Stats updated in database")
                    ...
                    if not overall_over_35_goals_stats.exists():
                        ...
                        self.log_info(f"Team {overall_team.name} Over 3.5 Goals Overall Stats created in database")
                    else:
                        ...
                        self.log_info(f"Team {overall_team.name} Over 3.5 Goals Overall Stats updated in database")
                ...
                for table_tr in table_trs:
                    ...
                    if not home_over_35_goals_stats.exists():
                        home_over_35_goals_stats.save()
                        self.log_info(f"Team {home_team.name} Over 3.5 Goals 1H Home Stats created in database")
                    else:
                        target_home_over_35_goals_stats: E5Over35GoalsStats = E5Over35GoalsStats.objects.get(
                            team=home_team)
                        target_home_over_35_goals_stats.home_matches_played = home_matches_played
                        ...
                        target_home_over_35_goals_stats.save()
                        self.log_info(f"Team {home_team.name} Over 3.5 Goals 1H Home Stats updated in database")
                    ...
                    if not away_over_35_goals_stats.exists():
                        away_over_35_goals_stats.save()
                        self.log_info(f"Team {away_team.name} Over 3.5 Goals 1H Away Stats created in database")
                    else:
                        target_away_over_35_goals_stats: E5Over35GoalsStats = E5Over35GoalsStats.objects.get(
                            team=away_team)
                        target_away_over_35_goals_stats.away_matches_played = away_matches_played
                        ...
                        target_away_over_35_goals_stats.save()
                        self.log_info(f"Team {away_team.name} Over 3.5 Goals 1H Away Stats updated in database")
                    ...
                    if not overall_over_35_goals_stats.exists():
                        overall_over_35_goals_stats.save()
                        self.log_info(f"Team {overall_team.name} Over 3.5 Goals 1H Overall Stats created in database")
                    else:
                        target_overall_over_35_goals_stats: E5Over35GoalsStats = E5Over35GoalsStats.objects.get(
                            team=overall_team)
                        target_overall_over_35_goals_stats.overall_matches_played = overall_matches_played
                        ...
                        target_overall_over_35_goals_stats.save()
                        self.log_info(f"Team {overall_team.name} Over 3.5 Goals 1H Overall Stats updated in database")
                ...
                for table_tr in table_trs:
                    ...
                    if not home_over_35_goals_stats.exists():
                        home_over_35_goals_stats.save()
                        self.log_info(f"Team {home_team.name} Over 3.5 Goals 2H Home Stats created in database")
                    else:
                        target_home_over_35_goals_stats: E5Over35GoalsStats = E5Over35GoalsStats.objects.get(
                            team=home_team)
                        target_home_over_35_goals_stats.home_matches_played = home_matches_played
                        ...
                        target_home_over_35_goals_stats.save()
                        self.log_info(f"Team {home_team.name} Over 3.5 Goals 2H Home Stats updated in database")
                    ...
                    if not away_over_35_goals_stats.exists():
                        away_over_35_goals_stats.save()
                        self.log_info(f"Team {away_team.name} Over 3.5 Goals 2H Away Stats created in database")
                    else:
                        target_away_over_35_goals_stats: E5Over35GoalsStats = E5Over35GoalsStats.objects.get(
                            team=away_team)
                        target_away_over_35_goals_stats.away_matches_played = away_matches_played
                        ...
                        target_away_over_35_goals_stats.save()
                        self.log_info(f"Team {away_team.name} Over 3.5 Goals 2H Away Stats updated in database")
                    ...
                    if not overall_over_35_goals_stats.exists():
                        overall_over_35_goals_stats.save()
                        self.log_info(f"Team {overall_team.name} Over 3.5 Goals 2H Overall Stats created in database")
                    else:
                        target_overall_over_35_goals_stats: E5Over35GoalsStats = E5Over35GoalsStats.objects.get(
                            team=overall_team)
                        target_overall_over_35_goals_stats.overall_matches_played = overall_matches_played
                        ...
                        target_overall_over_35_goals_stats.save()
                        self.log_info(f"Team {overall_team.name} Over 3.5 Goals 2H Overall Stats updated in database")
                ...
                for table_tr in table_trs:
                    ...
                    if not home_over_35_goals_stats.exists():
                        home_over_35_goals_stats.save()
                        self.log_info(f"Team {home_team.name} Over 3.5 Goals BH Home Stats created in database")
                    else:
                        target_home_over_35_goals_stats: E5Over35GoalsStats = E5Over35GoalsStats.objects.get(
                            team=home_team)
                        target_home_over_35_goals_stats.home_matches_played = home_matches_played
                        ...
                        target_home_over_35_goals_stats.save()
                        self.log_info(f"Team {home_team.name} Over 3.5 Goals BH Home Stats updated in database")
                    ...
                    if not away_over_35_goals_stats.exists():
                        away_over_35_goals_stats.save()
                        self.log_info(f"Team {away_team.name} Over 3.5 Goals BH Away Stats created in database")
                    else:
                        target_away_over_35_goals_stats: E5Over35GoalsStats = E5Over35GoalsStats.objects.get(
                            team=away_team)
                        target_away_over_35_goals_stats.away_matches_played = away_matches_played
                        ...
                        target_away_over_35_goals_stats.save()
                        self.log_info(f"Team {away_team.name} Over 3.5 Goals BH Away Stats updated in database")
                    ...
                    if not overall_over_35_goals_stats.exists():
                        overall_over_35_goals_stats.save()
                        self.log_info(f"Team {overall_team.name} Over 3.5 Goals BH Overall Stats created in database")
                    else:
                        target_overall_over_35_goals_stats: E5Over35GoalsStats = E5Over35GoalsStats.objects.get(
                            team=overall_team)
                        target_overall_over_35_goals_stats.overall_matches_played = overall_matches_played
                        ...
                        target_overall_over_35_goals_stats.save()
                        self.log_info(f"Team {overall_team.name} Over 3.5 Goals BH Overall Stats updated in database")
