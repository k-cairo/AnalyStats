...
from Website.models import E5Season, E5Over05GoalsIframe
    ...
    def get_iframes(self) -> None:
        ...
        if self.status.success:
            for season in self.ACTIVE_SEASONS:
                ...
                # Get Url
                self.get(url=f"{season.url}over-0-5-goals/", error_context=f"{self.ERROR_CONTEXT}.get_iframes()")
                ...
                # Get BTTS Iframes
                over_05_goals_iframes: ResultSet[Tag] = self.soup.select(selector="div.tab-content iframe")
                ...
                if not iframe.exists():
                    iframe.save()
                    self.log_info(f"League {season.league.name} Over 0.5 Goals Iframe created in database")
                else:
                    target_iframe: E5Over05GoalsIframe = E5Over05GoalsIframe.objects.get(season=season)
                    ...
                    target_iframe.save()
                    self.log_info(f"League {season.league.name} Over 0.5 Goals Iframe updated in database")
