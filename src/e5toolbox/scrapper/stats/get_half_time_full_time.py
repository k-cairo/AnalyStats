...
from Website.models import E5Season, E5HalfTimeFullTimeIframe
    ...
    def get_iframes(self) -> None:
        ...
        if self.status.success:
            for season in self.ACTIVE_SEASONS:
                ...
                # Get Url
                self.get(url=f"{season.url}half-time-full-time/", error_context=f"{self.ERROR_CONTEXT}.get_iframes()")
                if not self.status.success:
                    continue

                # Get Iframes
                ht_ft_iframe: Tag | None = self.soup.select_one(selector="div.fusion-text.fusion-text-2 iframe")

                # Check Iframes
                if ht_ft_iframe is None:
                ...
                # Get Iframe Url
                try:
                    iframe_url: str = ht_ft_iframe['src']
                ...
                # Get Iframe
                iframe: E5HalfTimeFullTimeIframe = E5HalfTimeFullTimeIframe()
                ...
                if not iframe.exists():
                    iframe.save()
                    self.log_info(f"League {season.league.name} Half Time / Full Time Iframe created in database")
                else:
                    target_iframe: E5HalfTimeFullTimeIframe = E5HalfTimeFullTimeIframe.objects.get(season=season)
                    target_iframe.url = iframe_url
                    target_iframe.save()
                    self.log_info(f"League {season.league.name} Half Time / Full Time Iframe updated in database")
