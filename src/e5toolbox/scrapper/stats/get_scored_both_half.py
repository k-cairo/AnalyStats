...
from Website.models import E5Season, E5ScoredBothHalfIframes
from e5toolbox.scrapper.E5SeleniumWebdriver import E5SeleniumWebDriver, E5SeleniumWebdriverError


# E5
@dataclasses.dataclass
class E5GetScoredBothHalf(E5SeleniumWebDriver):
    ERROR_CONTEXT: ClassVar[str] = "E5GetScoredBothHalf"
    ...
    def get_iframes(self) -> None:
        ...
        if self.status.success:
            for season in self.ACTIVE_SEASONS:
                ...
                # Get Url
                self.get(url=f"{season.url}sbh/", error_context=f"{self.ERROR_CONTEXT}.get_iframes()")
                ...
                # Get Iframe
                iframe: E5ScoredBothHalfIframes = E5ScoredBothHalfIframes()
                ...
                if not iframe.exists():
                    iframe.save()
                    self.log_info(f"League {season.league.name} Scored Both Half Iframe created in database")
                else:
                    target_iframe: E5ScoredBothHalfIframes = E5ScoredBothHalfIframes.objects.get(season=season)
                    ...
                    target_iframe.save()
                    self.log_info(f"League {season.league.name} Scored Both Half Iframe updated in database")
