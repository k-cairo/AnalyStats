...
from Website.models import E5Season, E5Over15GoalsIframe, E5Team, E5Over15GoalsStats
from e5toolbox.scrapper.E5SeleniumWebdriver import E5SeleniumWebDriver, E5SeleniumWebdriverError


# E5
@dataclasses.dataclass
class E5GetOver15Goals(E5SeleniumWebDriver):
    ERROR_CONTEXT: ClassVar[str] = "E5GetOver15Goals"
    ACTIVE_SEASONS: ClassVar[QuerySet[E5Season]] = E5Season.objects.filter(active=True)
    OVER_15_GOALS_IFRAMES: ClassVar[QuerySet[E5Over15GoalsIframe]] = E5Over15GoalsIframe.objects.filter(
        season__active=True)
    ...
    def get_iframes(self) -> None:
        ...
        if self.status.success:
            for season in self.ACTIVE_SEASONS:
                ...
                # Get Url
                self.get(url=f"{season.url}over-1-5-goals/", error_context=f"{self.ERROR_CONTEXT}.get_iframes()")
                if not self.status.success:
                    continue

                # Get BTTS Iframes
                over_15_goals_iframes: ResultSet[Tag] = self.soup.select(selector="div.tab-content iframe")
                ...
                if not iframe.exists():
                    iframe.save()
                    self.log_info(f"League {season.league.name} Over 1.5 Goals Iframe created in database")
                else:
                    target_iframe: E5Over15GoalsIframe = E5Over15GoalsIframe.objects.get(season=season)
                    ...
                    target_iframe.save()
                    self.log_info(f"League {season.league.name} Over 1.5 Goals Iframe updated in database")
    ...
    def parse_iframes(self) -> None:
        ...
        if self.status.success:
            for iframe in self.OVER_15_GOALS_IFRAMES:
                iframe: E5Over15GoalsIframe  # Type hinting for Intellij
