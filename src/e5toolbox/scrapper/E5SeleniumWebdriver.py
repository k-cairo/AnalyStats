...
class E5SeleniumWebdriverError(Enum):
    ...
    ERROR_TYPE_GET_ACTIVE_SEASON_NAME_OR_URL_FAILED = "get_active_season_name_or_url_failed"
    ERROR_TYPE_GET_ACTIVE_SEASON_IFRAME_URL_FAILED = "get_active_season_iframe_url_failed"
    ERROR_TYPE_GET_ACTIVE_SEASON_OVER_05_GOALS_IFRAME_URL_FAILED = "get_active_season_over_05_goals_iframe_url_failed"
    ERROR_TYPE_GET_ACTIVE_SEASON_OVER_15_GOALS_IFRAME_URL_FAILED = "get_active_season_over_15_goals_iframe_url_failed"
    ERROR_TYPE_GET_ACTIVE_SEASON_OVER_25_GOALS_IFRAME_URL_FAILED = "get_active_season_over_25_goals_iframe_url_failed"
    ERROR_TYPE_GET_ACTIVE_SEASON_OVER_35_GOALS_IFRAME_URL_FAILED = "get_active_season_over_35_goals_iframe_url_failed"
    ERROR_TYPE_GET_ACTIVE_SEASON_CORNER_IFRAME_URL_FAILED = "get_active_season_corner_iframe_url_failed"
    ERROR_TYPE_GET_ACTIVE_SEASON_CARD_IFRAME_URL_FAILED = "get_active_season_card_iframe_url_failed"
    ERROR_TYPE_IFRAME_EMPTY = "iframe_empty"
    ERROR_TYPE_GET_ACTIVE_SEASON_TEAMS_FAILED = "get_active_season_teams_failed"
    ERROR_TYPE_GET_ACTIVE_SEASON_TABLE_TR_FAILED = "get_active_season_table_tr_failed"
    ERROR_TYPE_GET_BTTS_IFRAMES_FAILED = "get_btts_iframes_failed"
    ERROR_TYPE_GET_CARDS_IFRAMES_FAILED = "get_cards_iframes_failed"
    ERROR_TYPE_BTTS_IFRAMES_BAD_LENGTH = "btts_iframes_bad_length"
    ...
    ERROR_TYPE_GET_ACTIVE_SEASON_TEAM_NAME_OR_TEAM_URL_FAILED = "get_active_season_team_name_or_team_url_failed"
    ERROR_TYPE_GET_ACTIVE_SEASON_TEAM_RANKING_FAILED = "get_active_season_team_ranking_failed"
...
###################################################################################################################################"
        ...
        # E5
    def get_active_seasons_teams(self) -> None:
        # Query League Tables (Active Seasons)
        active_league_table: QuerySet[E5LeagueTableIframe] = E5LeagueTableIframe.objects.filter(season__active=True)

        # Check connection
        self.check_is_connected()

        if self.status.success:
            error_context: str = "get_active_seasons_teams()"
            for league_table in active_league_table:
                league_table: E5LeagueTableIframe  # Type hinting for Intellij

                # Get Url
                try:
                    self.driver.get(league_table.url)
                except Exception as ex:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_URL_FAILED,
                                   error_context=error_context, exception=ex)
                    logging.warning(msg=f"Get Active Seasons Teams - {self.status.error_context} : "
                ...
                # Get Teams
                try:
                    teams_a: ResultSet[Tag] = soup.select(selector="table.waffle.no-grid tr td a[target='_blank']")
                except Exception as ex:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_ACTIVE_SEASON_TEAMS_FAILED,
                                   error_context=error_context, exception=ex)
                    logging.warning(msg=f"Get Active Seasons Teams - {self.status.error_context} : "
                                        f"{self.status.error_type} : {self.status.exception}")
                    continue
                ...
                for team_a in teams_a:
                    try:
                        ...
                        self.exception(
                            error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_ACTIVE_SEASON_TEAM_NAME_OR_TEAM_URL_FAILED,
                            error_context=error_context, exception=ex)
                        logging.warning(msg=f"Get Active Seasons Teams - {self.status.error_context} : "
                                            f"{self.status.error_type} : {self.status.exception}")
                        continue
        ...
        # E5
    def get_active_seasons_teams_ranking(self) -> None:
        # Query League Tables (Active Seasons)
        active_league_table: QuerySet[E5LeagueTableIframe] = E5LeagueTableIframe.objects.filter(season__active=True)
        ...
        if self.status.success:
            error_context: str = "get_active_seasons_teams_ranking()"
            for league_table in active_league_table:
                league_table: E5LeagueTableIframe  # Type hinting for Intellij
                ...
                try:
                    ...
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_URL_FAILED,
                                   error_context=error_context, exception=ex)
                    logging.warning(msg=f"Get Active Seasons Teams Ranking - {self.status.error_context} : "
                                        f"{self.status.error_type} : {self.status.exception}")
                    continue
                ...
                try:
                    ...
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_SOUP_FAILED,
                                   error_context=error_context, exception=ex)
                    logging.warning(msg=f"Active Seasons Teams Ranking - {self.status.error_context} : "
                                        f"{self.status.error_type} : {self.status.exception}")
                    continue
    ...
    ################################################## GET IFRAMES #####################################################
    # E5
    def get_active_seasons_league_table_iframes(self) -> None:
        # Query Active Seasons
        active_seasons: QuerySet[E5Season] = E5Season.objects.filter(active=True)

        # Check connection
        self.check_is_connected()

        if self.status.success:
            error_context: str = "get_active_seasons_league_table_iframes()"
            for season in active_seasons:
                season: E5Season  # Type hinting for Intellij

                # Get Url
                try:
                    self.driver.get(season.url)
                except Exception as ex:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_URL_FAILED,
                                   error_context=error_context, exception=ex)
                    continue
                ...
                try:
                    ...
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_SOUP_FAILED,
                                   error_context=error_context, exception=ex)
                    logging.warning(msg=f"Get Active Seasons League Table Iframes - {self.status.error_context} : "
                                        f"{self.status.error_type} : {self.status.exception}")
                    continue

                # Get League Table Iframe Url
                try:
                    iframe_url: str = soup.select_one(selector="div.fusion-text.fusion-text-2 iframe")['src']
                except Exception as ex:
    ...
    # E5
    def get_active_seasons_btts_iframes(self) -> None:
        # Variables
        btts_iframes: ResultSet[Tag] | None = None
        ...
        if self.status.success:
            error_context: str = "get_active_seasons_btts_iframes()"
            for active_season in active_seasons:
                active_season: E5Season  # Type hinting for Intellij

                # Get Url
                try:
                    self.driver.get(f"{active_season.url}btts/")
                except Exception as ex:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_URL_FAILED,
                                   error_context=error_context, exception=ex)
                    logging.warning(msg=f"Get Active Seasons BTTS Iframes - {self.status.error_context} : "
                                        f"{self.status.error_type} : {self.status.exception}")
                    continue
                ...
                # Get BTTS Iframes
                try:
                    btts_iframes = soup.select(selector="div.tab-content iframe")
                except Exception as ex:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_BTTS_IFRAMES_FAILED,
                                   error_context=error_context, exception=ex)
                    logging.warning(msg=f"Get Active Seasons BTTS Iframes - {self.status.error_context} : "
                                        f"{self.status.error_type} : {self.status.exception}")

                if len(btts_iframes) != 5:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_BTTS_IFRAMES_BAD_LENGTH,
                ...
                if not iframe.not_empty():
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_IFRAME_EMPTY,
                                   error_context=error_context, exception=None)
                    logging.warning(
                        msg=f"Get Active Seasons BTTS Iframes - {self.status.error_context} : "
                            f"{self.status.error_type} : {self.status.exception}")
                    continue
                ...
                if not iframe.exists():
                    ...
                    target_iframe: E5BttsIframes = E5BttsIframes.objects.get(season=active_season)
    ...
    # E5
    def get_active_seasons_over_05_goals_iframes(self) -> None:
        # Query Active Seasons
        active_seasons: QuerySet[E5Season] = E5Season.objects.filter(active=True)
        ...
        if self.status.success:
            error_context: str = "get_active_seasons_over_05_goals_iframes()"
            for active_season in active_seasons:
                active_season: E5Season  # Type hinting for Intellij

                # Get Url
                try:
                    self.driver.get(f"{active_season.url}over-0-5-goals/")
                except Exception as ex:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_URL_FAILED,
                                   error_context=error_context, exception=ex)
                    logging.warning(msg=f"Get Active Seasons Over 0.5 Goals Iframes - {self.status.error_context} : "
                                        f"{self.status.error_type} : {self.status.exception}")
                    continue
                ...
                try:
                    over_05_goals_iframes: ResultSet[Tag] = soup.select(selector="div.tab-content iframe")
                except Exception as ex:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_BTTS_IFRAMES_FAILED,
                                   error_context=error_context, exception=ex)
                    logging.warning(msg=f"Get Active Seasons Over 0.5 Goals Iframes - {self.status.error_context} : "
                ...
                if len(over_05_goals_iframes) != 4:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_BTTS_IFRAMES_BAD_LENGTH,
                                   error_context=error_context, exception=None)
                    logging.warning(msg=f"Get Active Seasons Over 0.5 Goals Iframes - {self.status.error_context} : "
                                        f"{self.status.error_type} : {self.status.exception}")
                ...
                # Get Overs 0.5 Goals Iframe
                iframe: E5Over05GoalsIframe = E5Over05GoalsIframe()
                iframe.season = active_season
                for idx, over_05_goals_iframe in enumerate(over_05_goals_iframes):
                    try:
                        iframe_url: str = over_05_goals_iframe['src']
                    except Exception as ex:
                        self.exception(
                            error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_ACTIVE_SEASON_OVER_05_GOALS_IFRAME_URL_FAILED,
                            error_context=error_context, exception=ex)
                        logging.warning(
                            msg=f"Get Active Seasons Over 0.5 Goals Iframes - {self.status.error_context} : "
                                f"{self.status.error_type} : {self.status.exception}")
                        continue
                ...
                if not iframe.exists():
                    ...
                    target_iframe: E5Over05GoalsIframe = E5Over05GoalsIframe.objects.get(season=active_season)
    ...
    # E5
    def get_active_seasons_over_15_goals_iframes(self) -> None:
        # Query Active Seasons
        active_seasons: QuerySet[E5Season] = E5Season.objects.filter(active=True)
        ...
        if self.status.success:
            error_context: str = "get_active_seasons_over_15_goals_iframes()"
            for active_season in active_seasons:
                active_season: E5Season  # Type hinting for Intellij

                # Get Url
                try:
                    self.driver.get(f"{active_season.url}over-1-5-goals/")
                except Exception as ex:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_URL_FAILED,
                                   error_context=error_context, exception=ex)
                    logging.warning(msg=f"Get Active Seasons Over 1.5 Goals Iframes - {self.status.error_context} : "
                                        f"{self.status.error_type} : {self.status.exception}")
                    continue
                ...
                try:
                    over_15_goals_iframes: ResultSet[Tag] = soup.select(selector="div.tab-content iframe")
                except Exception as ex:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_BTTS_IFRAMES_FAILED,
                                   error_context=error_context, exception=ex)
                    logging.warning(msg=f"Get Active Seasons Over 1.5 Goals Iframes - {self.status.error_context} : "
                ...
                if len(over_15_goals_iframes) != 4:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_BTTS_IFRAMES_BAD_LENGTH,
                                   error_context=error_context, exception=None)
                    logging.warning(msg=f"Get Active Seasons Over 1.5 Goals Iframes - {self.status.error_context} : "
                                        f"{self.status.error_type} : {self.status.exception}")
                ...
                # Get Overs 1.5 Goals Iframe
                iframe: E5Over15GoalsIframe = E5Over15GoalsIframe()
                iframe.season = active_season
                for idx, over_15_goals_iframe in enumerate(over_15_goals_iframes):
                    try:
                        ...
                        self.exception(
                            ...
                            error_context=error_context, exception=ex)
                        logging.warning(
                            msg=f"Get Active Seasons Over 1.5 Goals Iframes - {self.status.error_context} : "
                                f"{self.status.error_type} : {self.status.exception}")
                        continue
                ...
                if not iframe.exists():
                    ...
                    target_iframe: E5Over15GoalsIframe = E5Over15GoalsIframe.objects.get(season=active_season)
    ...
    # E5
    def get_active_seasons_over_25_goals_iframes(self) -> None:
        # Query Active Seasons
        active_seasons: QuerySet[E5Season] = E5Season.objects.filter(active=True)
        ...
        if self.status.success:
            error_context: str = "get_active_seasons_over_25_goals_iframes()"
            for active_season in active_seasons:
                active_season: E5Season  # Type hinting for Intellij

                # Get Url
                try:
                    self.driver.get(f"{active_season.url}over-2-5-goals/")
                except Exception as ex:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_URL_FAILED,
                                   error_context=error_context, exception=ex)
                    logging.warning(msg=f"Get Active Seasons Over 2.5 Goals Iframes - {self.status.error_context} : "
                                        f"{self.status.error_type} : {self.status.exception}")
                    continue
                ...
                try:
                    over_25_goals_iframes: ResultSet[Tag] = soup.select(selector="div.tab-content iframe")
                except Exception as ex:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_BTTS_IFRAMES_FAILED,
                                   error_context=error_context, exception=ex)
                    logging.warning(msg=f"Get Active Seasons Over 2.5 Goals Iframes - {self.status.error_context} : "
                ...
                for idx, over_25_goals_iframe in enumerate(over_25_goals_iframes):
                    try:
                        ...
                        self.exception(
                            ...
                            error_context=error_context, exception=ex)
                        logging.warning(
                            msg=f"Get Active Seasons Over 2.5 Goals Iframes - {self.status.error_context} : "
                                f"{self.status.error_type} : {self.status.exception}")
                        continue
                ...
                if not iframe.exists():
                    ...
                    target_iframe: E5Over25GoalsIframe = E5Over05GoalsIframe.objects.get(season=active_season)
    ...
    # E5
    def get_active_seasons_over_35_goals_iframes(self) -> None:
        # Query Active Seasons
        active_seasons: QuerySet[E5Season] = E5Season.objects.filter(active=True)
        ...
        if self.status.success:
            error_context: str = "get_active_seasons_over_35_goals_iframes()"
            for active_season in active_seasons:
                active_season: E5Season  # Type hinting for Intellij

                # Get Url
                try:
                    self.driver.get(f"{active_season.url}over-3-5-goals/")
                except Exception as ex:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_URL_FAILED,
                                   error_context=error_context, exception=ex)
                    logging.warning(msg=f"Get Active Seasons Over 3.5 Goals Iframes - {self.status.error_context} : "
                                        f"{self.status.error_type} : {self.status.exception}")
                    continue
                ...
                try:
                    over_35_goals_iframes: ResultSet[Tag] = soup.select(selector="div.tab-content iframe")
                except Exception as ex:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_BTTS_IFRAMES_FAILED,
                                   error_context=error_context, exception=ex)
                    logging.warning(msg=f"Get Active Seasons Over 3.5 Goals Iframes - {self.status.error_context} : "
                ...
                if len(over_35_goals_iframes) != 4:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_BTTS_IFRAMES_BAD_LENGTH,
                                   error_context=error_context, exception=None)
                    logging.warning(msg=f"Get Active Seasons Over 3.5 Goals Iframes - {self.status.error_context} : "
                                        f"{self.status.error_type} : {self.status.exception}")
                ...
                # Get Overs 3.5 Goals Iframe
                iframe: E5Over35GoalsIframe = E5Over35GoalsIframe()
                iframe.season = active_season
                for idx, over_35_goals_iframe in enumerate(over_35_goals_iframes):
                    try:
                        iframe_url: str = over_35_goals_iframe['src']
                    except Exception as ex:
                        self.exception(
                            error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_ACTIVE_SEASON_OVER_35_GOALS_IFRAME_URL_FAILED,
                            error_context=error_context, exception=ex)
                        logging.warning(
                            msg=f"Get Active Seasons Over 3.5 Goals Iframes - {self.status.error_context} : "
                                f"{self.status.error_type} : {self.status.exception}")
                        continue
                ...
                if not iframe.exists():
                    ...
                    target_iframe: E5Over35GoalsIframe = E5Over35GoalsIframe.objects.get(season=active_season)
    ...
    # E5
    def get_active_seasons_win_draw_loss_percentage_iframes(self) -> None:
        # Variables
        btts_iframes: ResultSet[Tag] | None = None
    ...
    # E5
    def get_active_seasons_corners_iframes(self) -> None:
        # Query Active Seasons
        active_seasons: QuerySet[E5Season] = E5Season.objects.filter(active=True)
        ...
        if self.status.success:
            error_context: str = "get_active_seasons_corners_iframes()"
            for active_season in active_seasons:
                active_season: E5Season  # Type hinting for Intellij

                # Get Url
                try:
                    self.driver.get(f"{active_season.url}corners/")
                except Exception as ex:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_URL_FAILED,
                                   error_context=error_context, exception=ex)
                    logging.warning(msg=f"Get Active Seasons Corners Iframes - {self.status.error_context} : "
                                        f"{self.status.error_type} : {self.status.exception}")
                    continue
                ...
                # Get Corners Iframes
                try:
                    corners_iframes: ResultSet[Tag] = soup.select(selector="div.tab-content iframe")
                except Exception as ex:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_BTTS_IFRAMES_FAILED,
                                   error_context=error_context, exception=ex)
                    logging.warning(msg=f"Get Active Seasons Corners Iframes - {self.status.error_context} : "
                ...
                if len(corners_iframes) != 9:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_CORNERS_IFRAMES_BAD_LENGTH,
                                   error_context=error_context, exception=None)
                    logging.warning(msg=f"Get Active Seasons Corners Iframes - {self.status.error_context} : "
                                        f"{self.status.error_type} : {self.status.exception}")
                    continue
                ...
                for idx, corner_iframe in enumerate(corners_iframes):
                    try:
                        ...
                        self.exception(
                            error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_ACTIVE_SEASON_CORNER_IFRAME_URL_FAILED,
                            error_context=error_context, exception=ex)
                        logging.warning(msg=f"Get Active Seasons Corners Iframes - {self.status.error_context} : "
                                            f"{self.status.error_type} : {self.status.exception}")
                        continue
                ...
                if not iframe.exists():
                    ...
                    target_iframe: E5CornersIframes = E5CornersIframes.objects.get(season=active_season)
    ...
    # E5
    def get_active_seasons_cards_iframes(self) -> None:
        # Query Active Seasons
        active_seasons: QuerySet[E5Season] = E5Season.objects.filter(active=True)
        ...
        if self.status.success:
            error_context: str = "get_active_seasons_cards_iframes()"
            for active_season in active_seasons:
                active_season: E5Season  # Type hinting for Intellij

                # Get Url
                try:
                    self.driver.get(f"{active_season.url}cards/")
                except Exception as ex:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_URL_FAILED,
                                   error_context=error_context, exception=ex)
                    logging.warning(msg=f"Get Active Seasons Cards Iframes - {self.status.error_context} : "
                                        f"{self.status.error_type} : {self.status.exception}")
                    continue
                ...
                # Get Cards Iframes
                try:
                    cards_iframes: ResultSet[Tag] = soup.select(selector="div.tab-content iframe")
                except Exception as ex:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_CARDS_IFRAMES_FAILED,
                                   error_context=error_context, exception=ex)
                    logging.warning(msg=f"Get Active Seasons Cards Iframes - {self.status.error_context} : "
                                        f"{self.status.error_type} : {self.status.exception}")
                    continue
                ...
                if len(cards_iframes) != 4:
                    self.exception(error_type=E5SeleniumWebdriverError.ERROR_TYPE_CARDS_IFRAMES_BAD_LENGTH,
                                   error_context=error_context, exception=None)
                    logging.warning(msg=f"Get Active Seasons Cards Iframes - {self.status.error_context} : "
                                        f"{self.status.error_type} : {self.status.exception}")
                    continue
                ...
                for idx, card_iframe in enumerate(cards_iframes):
                    try:
                        ...
                        self.exception(
                            error_type=E5SeleniumWebdriverError.ERROR_TYPE_GET_ACTIVE_SEASON_CARD_IFRAME_URL_FAILED,
                            error_context=error_context, exception=ex)
                        logging.warning(msg=f"Get Active Seasons Cards Iframes - {self.status.error_context} : "
                                            f"{self.status.error_type} : {self.status.exception}")
                        continue
                ...
                if not iframe.exists():
                    ...
                    target_iframe: E5CardsIframes = E5CardsIframes.objects.get(season=active_season)
