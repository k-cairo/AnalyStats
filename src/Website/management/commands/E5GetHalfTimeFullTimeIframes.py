...
from Website.models import E5HalfTimeFullTimeIframe
from e5toolbox.scrapper.E5SeleniumWebdriver import E5SeleniumWebDriver


# E5
class Command(BaseCommand):
    CONTEXT: ClassVar[str] = "GetHalfTimeFullTimeIframes"
    help = "Get Half Time Full Time Iframes"
    ...
    def handle(self, *args, **options):
        ...
        if scraper.status.success:
            scraper.get_iframe(endpoint='half-time-full-time/', error_context=self.CONTEXT,
                               save_message="Half Time Full Time Iframes", class_=E5HalfTimeFullTimeIframe)
            if not scraper.status.success:
                scraper.log_warning(f"{self.CONTEXT} - {scraper.status.error_context} : {scraper.status.error_type} : "
                                    f"{scraper.status.exception}")
        ...
        self.stdout.write("Half Time Full Time Iframes Updated Successfully")
