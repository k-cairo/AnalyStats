...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Logging
        logging.info(msg=f"{datetime.now()} : GetActiveSeasonsTeams start -----")

        # Init driver
        selenium_webdriver.init()
        if not selenium_webdriver.status.success:
            logging.warning(msg=f"Get Active Seasons Teams - {selenium_webdriver.status.error_context} : "
        ...
        if selenium_webdriver.status.success:
            selenium_webdriver.get_active_seasons_teams()
            if not selenium_webdriver.status.success:
                logging.warning(msg=f"Get Active Seasons Teams - {selenium_webdriver.status.error_context} : "
                                    f"{selenium_webdriver.status.error_type} : "
                                    f"{selenium_webdriver.status.exception}")
        ...
        # Logging
        logging.info(msg=f"{datetime.now()} : GetActiveSeasonsTeams end -----")
