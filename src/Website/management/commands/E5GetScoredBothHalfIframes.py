...
from Website.models import E5ScoredBothHalfIframes
from e5toolbox.scrapper.E5SeleniumWebdriver import E5SeleniumWebDriver


# E5
class Command(BaseCommand):
    CONTEXT: ClassVar[str] = "GetScoredBothHalfIframes"
    help = "Get Scored Both Half Iframes"
    ...
    def handle(self, *args, **options):
        ...
        if scraper.status.success:
            scraper.get_iframes(endpoint='sbh/', error_context=self.CONTEXT, iframe_length=2,
                                save_message="Scored Both Half Iframes", class_=E5ScoredBothHalfIframes)
            if not scraper.status.success:
                scraper.log_warning(f"{self.CONTEXT} - {scraper.status.error_context} : {scraper.status.error_type} : "
                                    f"{scraper.status.exception}")
        ...
        self.stdout.write("Scored Both Half Iframes Updated Successfully")
