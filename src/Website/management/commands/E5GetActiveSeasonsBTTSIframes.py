...
class Command(BaseCommand):
    help = "Get Active Season's BTTS Iframes"
    ...
    def handle(self, *args, **options):
        ...
        # Logging
        logging.info(msg=f"{datetime.now()} : GetActiveSeasonsBTTSIframes start -----")

        # Init driver
        selenium_webdriver.init()
        if not selenium_webdriver.status.success:
            logging.warning(msg=f"Get Active Seasons BTTS Iframes - {selenium_webdriver.status.error_context} : "
        ...
        if selenium_webdriver.status.success:
            selenium_webdriver.get_active_seasons_btts_iframes()
            if not selenium_webdriver.status.success:
                logging.warning(msg=f"Get Active Seasons BTTS Iframes - {selenium_webdriver.status.error_context} : "
                                    f"{selenium_webdriver.status.error_type} : "
                                    f"{selenium_webdriver.status.exception}")
        ...
        # Logging
        logging.info(msg=f"{datetime.now()} : GetActiveSeasonsBTTSIframes end -----")

        self.stdout.write("Active Season's BTTS Iframes Updated Successfully")
