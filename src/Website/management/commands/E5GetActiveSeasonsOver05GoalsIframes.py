...
class Command(BaseCommand):
    help = "Get Active Season's Overs 0.5 Goals Iframes"
    ...
    def handle(self, *args, **options):
        ...
        # Logging
        logging.info(msg=f"{datetime.now()} : GetActiveSeasonsOvers0.5GoalsIframes start -----")

        # Init driver
        selenium_webdriver.init()
        if not selenium_webdriver.status.success:
            logging.warning(msg=f"Get Active Seasons Over 0.5 Goals Iframes - {selenium_webdriver.status.error_context} : "
        ...
        if selenium_webdriver.status.success:
            selenium_webdriver.get_active_seasons_over_05_goals_iframes()
            if not selenium_webdriver.status.success:
                logging.warning(msg=f"Get Active Seasons Over 0.5 Goals Iframes - {selenium_webdriver.status.error_context} : "
                                    f"{selenium_webdriver.status.error_type} : "
                                    f"{selenium_webdriver.status.exception}")
        ...
        # Logging
        logging.info(msg=f"{datetime.now()} : GetActiveSeasonsOvers0.5GoalsIframes end -----")

        self.stdout.write("Active Season's Overs 0.5 Goals Iframes Updated Successfully")
